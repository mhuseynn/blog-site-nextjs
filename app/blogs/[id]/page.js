...
// //         <span onClick={handleSearchSubmit}  className="cursor-pointer">{blog.author?.email}</span>
...
// import { useParams } from "next/navigation";  
// import { useEffect, useState } from "react";
// import { useRouter } from "next/router";  // Change to `next/router` for client-side navigation
...
export default function BlogDetailPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await fetch(`/api/blogs/${id}`);  // Fetch the blog by its ID
        const data = await res.json();
        setBlog(data);  // Set the fetched blog
      } catch (err) {
        console.error("Failed to load blog", err);
      } finally {
        setLoading(false);
    ...
    if (id) fetchBlog();  // Fetch blog details when the ID is available
  }, [id]);
